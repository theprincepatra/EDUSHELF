...
form.addEventListener('submit', async function (event) {
    ...
    if (username === "") {
        ...
        showError(usernameInput, "Username must be within 15 characters");
        isValid = false;
    } else if (/^[0-9]+$/.test(username)) {
        showError(usernameInput, "Username cannot contain only numbers");
        isValid = false;
    } else if (!/^[A-Za-z0-9_.]+$/.test(username)) {
        showError(usernameInput, "Only letters, numbers, _ and . are allowed");
        isValid = false;
    }
